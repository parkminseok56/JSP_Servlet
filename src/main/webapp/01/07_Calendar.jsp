...
<%
   ...
   Calendar eDay = Calendar.getInstance(); // 출력될 달력의 말 일자가 저장될 객체
...
%>   
	<!-- jsp 문법을 이용해서 토요일부터 시작하는 4월 달력을 표시하세요  -->
	<table width="560" align="left" cellspacing="1"	bgcolor="black">
	     <tr bgcolor="white" height="50">
          ...
          <tr bgcolor="white" height="50"> <!-- 날짜가 표시되는 tr  -->
           <%
           int i;
           for ( i =1; i<START_WEEK ; i++) { //   START_WEEK =6
           		out.print("<td>&nbsp;</td>");
           		} // 1일자 전 빈칸들 (2023년 4월 1일은 토요일 -> 여섯칸 빈칸)  
           
			for (int d = 1; d <= lastDay; d++) { // lastDay= 30, 1일부터 30일까지 한 칸(td 테그)씩 표시
				out.print("<td>" + d + "</td>");
				if( i++ % 7 == 0){ // 7의 배수만큼 열이 채워지는 순간 줄 바꿈 실행. -> tr끝 tr 시작
					out.print("</tr><tr bgcolor=\"white\" height=\"50\">");
				}
			}
			for(int k =1; k<= (8-(i%7)); k++){
				out.print("<td>&nbsp;</td>");
			}   // 마지막 말 일 표시 후 남는 칸에 빈칸으로 
		 %>                
           </tr>
    </table>
